...
class EvaluationDatasetAndExportTests(unittest.TestCase):
    def test_synthetic_dataset_coverage(self):
        ...
        self.assertEqual({ticket.id for ticket in tickets}, {f"T{index:02d}" for index in range(1, 51)})
        self.assertEqual(len({(ticket.title, ticket.description) for ticket in tickets}), 50)
        for ticket in tickets:
            with self.subTest(ticket=ticket.id):
                self.assertTrue(ticket.title.strip())
                self.assertTrue(ticket.description.strip())
                self.assertEqual(ticket.expected.recommended_team, TEAMS[ticket.expected.category])
        # Labels are authored from the business policy, not generated by the mock.
        for category in CATEGORIES:
            self.assertGreaterEqual(sum(ticket.expected.category == category for ticket in tickets), 5)
        for priority in PRIORITIES:
            self.assertGreaterEqual(sum(ticket.expected.priority == priority for ticket in tickets), 5)
    ...
    def test_complete_two_model_export_continues_after_failures(self):
        ...
        responses = [URLError("Connection refused"), "bad JSON"] + [chat_response(assessment_data())] * 98
        try:
            ...
            for model in benchmark.MODELS:
                self.assertIn(model + ": 50 attempts", stdout.getvalue())
